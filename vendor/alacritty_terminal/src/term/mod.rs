...
pub struct Config {
    ...
    /// Whether East Asian Ambiguous characters should be treated as double-width.
    ///
    /// When `false`, ambiguous characters use standard width semantics.
    /// When `true`, ambiguous characters use CJK width semantics.
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn input(&mut self, c: char) {
        ...
        let width = match if self.config.ambiguous_wide {
            c.width_cjk()
        } else {
            c.width()
        } {
